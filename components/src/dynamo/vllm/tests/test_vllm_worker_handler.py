...
@pytest.mark.asyncio(loop_scope="function")
class TestExtractMultimodalData:
    """Tests for BaseWorkerHandler._extract_multimodal_data multimodal branching."""

    async def test_kimi_uses_raw_vision_chunk_path(self):
        handler = _make_decode_handler(
            model="moonshotai/Kimi-K2.5",
            disaggregation_mode="AGGREGATED",
        )
        ...
        result = await handler._extract_multimodal_data(
            _make_raw_frontend_request(image_urls=["http://img.png"]),
            "req-1",
            MagicMock(),
        )
