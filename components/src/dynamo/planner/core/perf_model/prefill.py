...
class PrefillRegressionModel(_BaseRegressionModel):
    ...
    def find_best_engine_prefill_thpt(
        self, ttft_sla: float, isl: float
    ) -> tuple[float, float]:
        """Find prefill engine throughput under a TTFT target.

        Predicts wall_time for a single prefill at the given ISL.
        If the predicted TTFT exceeds the SLA, logs a warning and
        returns the throughput anyway (caller decides how to handle).

        Args:
            ttft_sla: target TTFT in milliseconds.
            isl: input sequence length (tokens).

        Returns:
            (engine_thpt, actual_ttft_ms) -- prefill tokens/s per engine
            and predicted TTFT at that ISL.
        """
        ...
        return (engine_thpt, actual_ttft_ms)
