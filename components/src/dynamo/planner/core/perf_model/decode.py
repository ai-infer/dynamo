...
class DecodeRegressionModel(_BaseRegressionModel):
    ...
    def find_best_engine_decode_thpt(
        self, itl: float, context_length: float
    ) -> tuple[float, float]:
        """Find the maximum decode throughput (tokens/s) within an ITL target.

        Binary searches over batch_size at the given context_length for the
        maximum batch_size where predicted wall_time * 1000 <= itl.  Warns
        if even batch_size=1 violates the target.

        Args:
            itl: target ITL in milliseconds.
            context_length: average context length per decode request.

        Returns:
            (engine_thpt, actual_itl_ms) -- output tokens/s per engine
            and the predicted ITL at that operating point.
        """
        if not self._ensure_fitted() or context_length <= 0:
            return (1.0, 0.0)
        ...
        if best_wt * 1000.0 > itl:
            ...
            return (best_bs / best_wt, best_wt * 1000.0)
        ...
        engine_thpt = best_bs / best_wt
        return (engine_thpt, best_wt * 1000.0)
