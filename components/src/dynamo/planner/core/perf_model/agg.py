...
class AggRegressionModel(_BaseRegressionModel):
    ...
    def find_best_engine_agg_thpt(
        ...
        """Find the maximum agg engine throughput under both SLA targets.
        ...
        Returns:
            (engine_thpt, actual_ttft_ms, actual_itl_ms) -- output
            tokens/s per engine, and the predicted TTFT/ITL at the best
            operating point.  If even batch_size=1 violates an SLA,
            returns throughput at batch_size=1 with a warning (caller
        ...
        for bs in range(1, max_bs + 1):
            ...
            if itl_ms > itl_sla or ttft_ms > ttft_sla:
                if bs == 1:
                    ...
                    return (1.0 / wt, ttft_ms, itl_ms)
                break

            best_engine_thpt = bs / wt
            best_ttft_ms = ttft_ms
            best_itl_ms = itl_ms

        return (best_engine_thpt, best_ttft_ms, best_itl_ms)
