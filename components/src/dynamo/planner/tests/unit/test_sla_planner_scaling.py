...
PREFILL_ENGINE_THPT = 10000.0
DECODE_ENGINE_THPT = 500.0
...
def _build_planners(config, prometheus_client):
    ...
    prefill_planner.ttft_regression.find_best_engine_prefill_thpt.return_value = (
        PREFILL_ENGINE_THPT,
        75.0,
    )
    ...
    decode_planner.itl_regression.find_best_engine_decode_thpt.return_value = (
        DECODE_ENGINE_THPT,
...
def _expected_prefill(config, prefill_planner, sample):
    ...
    engine_thpt, _ = prefill_planner.ttft_regression.find_best_engine_prefill_thpt(
        ttft_sla=config.ttft, isl=sample["isl"]
    )
    expected = math.ceil(pred_prefill_throughput / engine_thpt)
    return max(expected, config.min_endpoint)


def _expected_decode(config, decode_planner, sample):
    engine_thpt, _ = decode_planner.itl_regression.find_best_engine_decode_thpt(
    ...
    expected = math.ceil(pred_decode_throughput / engine_thpt)
    return max(expected, config.min_endpoint)
