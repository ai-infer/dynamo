...
class TestPrefillRegressionModel:
    ...
    def test_find_best_engine_prefill_thpt(self):
        model = PrefillRegressionModel(
            max_num_fpm_samples=50, min_observations=3, bucket_count=16
        )
        for tokens in [500, 1000, 1500, 2000, 2500]:
        ...
        thpt, actual_ttft_ms = model.find_best_engine_prefill_thpt(
            ttft_sla=2000.0, isl=1000.0
        )
        assert thpt > 0
        assert 500 < thpt < 1500
        assert actual_ttft_ms > 0
        assert 1000 < actual_ttft_ms < 2000

    def test_find_best_engine_prefill_thpt_zero_isl(self):
        model = PrefillRegressionModel(
            max_num_fpm_samples=50, min_observations=3, bucket_count=16
        )
        for tokens in [500, 1000, 1500]:
            ...
            model.add_observation(fpm)
        thpt, _ = model.find_best_engine_prefill_thpt(ttft_sla=1000.0, isl=0.0)
        assert thpt == 1.0
...
class TestDecodeRegressionModel:
    ...
    def test_find_best_engine_decode_thpt(self):
        model = DecodeRegressionModel(
            max_num_fpm_samples=50, min_observations=3, bucket_count=16
        )
        self._train_thpt_model(model)

        thpt, actual_itl = model.find_best_engine_decode_thpt(
            itl=50.0, context_length=1000.0
        )
        assert thpt > 0
        assert actual_itl > 0
        assert actual_itl <= 50.0

    def test_find_best_engine_decode_thpt_zero_context(self):
        model = DecodeRegressionModel(
            max_num_fpm_samples=50, min_observations=3, bucket_count=16
        )
        self._train_2d(model)
        thpt, itl_ms = model.find_best_engine_decode_thpt(itl=50.0, context_length=0.0)
        assert thpt == 1.0
        assert itl_ms == 0.0
...
class TestAggRegressionModel:
    ...
    def test_find_best_engine_agg_thpt(self):
        model = AggRegressionModel(
            max_num_fpm_samples=50, min_observations=3, bucket_count=16
        )
        self._train_agg(model)

        thpt, actual_ttft, actual_itl = model.find_best_engine_agg_thpt(
            isl=2048.0,
            osl=150.0,
    ...
    def test_find_best_engine_agg_thpt_insufficient_data(self):
        model = AggRegressionModel(
            max_num_fpm_samples=50, min_observations=5, bucket_count=16
        )
        thpt, _, _ = model.find_best_engine_agg_thpt(
            isl=2048.0,
            osl=150.0,
            max_num_batched_tokens=4096,
            ttft_sla=500.0,
