...
def planner():
    ...
    with patch("dynamo.planner.monitoring.planner_metrics.Gauge") as mock_gauge:
        ...
        # Mock prefill regression (engine-level throughput)
        planner.ttft_regression = Mock()
        planner.ttft_regression.find_best_engine_prefill_thpt.return_value = (
            40000.0,
            75.0,
        )
        planner.ttft_regression.has_sufficient_data.return_value = True

        # Mock decode regression (engine-level throughput)
        planner.itl_regression = Mock()
        planner.itl_regression.find_best_engine_decode_thpt.return_value = (
            10000.0,
            9.5,
        )
...
class TestReplicaCalculation:
    ...
    def test_prefill_replica_calculation_basic(self, planner):
        ...
        # Formula: ceil(num_req * isl / interval / engine_thpt)
        pred_prefill_demand = (
            next_num_req * next_isl / planner.config.throughput_adjustment_interval
        )
        expected_prefill_replicas = math.ceil(pred_prefill_demand / engine_thpt)
    ...
    def test_decode_replica_calculation_basic(self, planner):
        ...
        # Formula: ceil(num_req * osl / interval / engine_thpt)
    ...
    def test_multi_gpu_engines(self, planner):
        ...
        # Engine-level throughput (already accounts for multi-GPU)
        planner.ttft_regression.find_best_engine_prefill_thpt.return_value = (
            40000.0,
            75.0,
        )
        ...
        # No engine_num_gpu division — regression returns engine-level throughput
        expected_prefill_replicas = math.ceil(
            20 * 3000 / planner.config.throughput_adjustment_interval / 40000.0
        )
        expected_decode_replicas = math.ceil(
            20 * 150 / planner.config.throughput_adjustment_interval / 5000.0
        )
