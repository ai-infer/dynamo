...
async def fetch_pre_deployment_metrics(
    ...
    if profile_results_dir:
        try:
            fpms = _convert_profiling_npz_to_fpms(profile_results_dir, component_type)
            if fpms:
                logger.info(
...
async def _try_endpoint(
    ...
    try:
        endpoint = runtime.endpoint(  # type: ignore[union-attr]
...
def _convert_profiling_npz_to_fpms(
    profile_results_dir: str,
    component_type: SubComponentType,
) -> list[ForwardPassMetrics]:
    """Convert legacy profiler npz arrays to synthetic ForwardPassMetrics."""
    fpms: list[ForwardPassMetrics] = []

    if component_type in (SubComponentType.PREFILL,):
        fpms.extend(_convert_prefill_npz(profile_results_dir))
    if component_type in (SubComponentType.DECODE,):
        fpms.extend(_convert_decode_npz(profile_results_dir))

    return fpms


def _convert_prefill_npz(profile_results_dir: str) -> list[ForwardPassMetrics]:
...
def _convert_decode_npz(profile_results_dir: str) -> list[ForwardPassMetrics]:
    npz_path = os.path.join(
